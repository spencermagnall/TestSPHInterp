...
subroutine interpolate3D(xyzh,weight,npart, &
                             xmin,datsmooth,nnodes,dxgrid,normalise,dat,ngrid,periodic,vertexcen)
 ...
 logical,      intent(in)  :: normalise, periodic, vertexcen
...
!  xminpix(:) = xmin(:) - 0.5*dxcell(:)
 ...
 !$omp shared(npart,xyzh,dat,datsmooth,datnorm,periodic,vertexcen)  &
